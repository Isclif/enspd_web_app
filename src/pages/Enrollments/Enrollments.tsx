...
import DataCoursEnrollments from "../../components/Gallery/DataCoursEnrollments";
...
const Enrollments = () => {
    ...
    try {
        const response = await fetch(`${URLS.API_BACK}/enrollments/`, {
            method: "GET",
            headers: headersList
        });
        if (!response.ok) {
    ...
    let dataTableFiltered = data?.filter((item) =>
        (searchTerm.name && item.course_data.name.toLowerCase().includes(searchTerm.name.toLowerCase())) ||
        (searchTerm.speciality_name && item.course_data.speciality.toLowerCase().includes(searchTerm.speciality_name.toLowerCase()))
    );
    ...
    return (
        <div>
            <Breadcrumb pageName="Liste de mes cours suivi" />
            ...
            <div className="w-full bg-white dark:bg-gray-800 shadow-md rounded-lg p-5 mb-6 dark:bg-boxdark">
                ...
                <div className="flex flex-col gap-10">
                    <DataCoursEnrollments data={data} onView={handleView} />
                </div>
