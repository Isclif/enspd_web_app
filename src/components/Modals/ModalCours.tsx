...
const ModalCours: React.FC<ModalFormProps> = ({
    showModal,
    setShowModal,
    onSubmit,
    title,
    ...
    const {token} = AuthUser()
        ...
        const [dataInstructor, setDataInstructor] = React.useState<DataItem[]>([]);
        ...
        const handleFormSubmit = (e: React.FormEvent) => {
            ...
            if (!formData.name) return window.alert("le nom du cours est requis")
            // if (!formData.description) return window.alert("la description n'est pas forcement requise")
            if (!selectedSpeciality) return window.alert("la spécialité est requise")
            if (!selectedInstructor) return window.alert("veuillez selectionner le professeur du cours")
    
            onSubmit({ ...formData, instructor: selectedInstructor as string, speciality: selectedSpeciality as string});
            ...
            <div className="bg-white p-6 rounded-lg shadow-xl max-w-2xl w-full transform scale-100 dark:bg-boxdark">
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                    ...
                    <div className="flex-none px-3">
                        <label className="block text-gray-700 text-sm font-bold mb-2">Professeur</label>
                        <select 
                        name="instructor"
                    ...
                    <div className="flex-none px-3">
                        <label className="block text-gray-700 text-sm font-bold mb-2">Spécialite</label>
                        <select 
                        name="speciality"
