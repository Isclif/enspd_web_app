...
const DataCours: React.FC<DataTableProps> = ({data, onView}) => {
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white px-5 pb-5 shadow-default dark:border-strokedark dark:bg-boxdark">
        {
            data?.length === 0 ? <div className="h-24 flex justify-center items-center">Pas de cours pour l'instant.</div> 
            : 
            <div className="max-w-full pt-5 flex flex-col md:flex-row md:flex-wrap gap-2">
                {data?.map((elm) =>(
                    <Link to={`/cours/${elm.id}`} key={elm.id} >
                        <div className="p-2 flex border-zinc-300 border rounded-md hover:cursor-pointer hover:bg-yellow-200">
                            <div className="h-36 min-w-36 w-auto bg-zinc-50 rounded-md p-2 dark:bg-boxdark">
                                <div className="font-bold pb-1">{elm.name}</div>
                                <div className=""><span>Specialité : </span>{elm.speciality_data.name}</div>
                                <div className="break-all sm:block hidden"><span>Département : </span>{truncateText(elm.speciality_data.dept_name, 20)}</div>
                                <div className="pt-1"><span>Profésseur : </span>{elm.instructor_name}</div>
                            </div>
